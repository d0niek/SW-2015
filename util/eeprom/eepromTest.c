...
//  printf("\nTest #1 - write string '%s' to address 0x0000", testString1);
    ...
    if (errorCode == I2C_CODE_OK);//    printf("\n        - done (status code OK)");
    else {
        printf("\nTest #1 - write string '%s' to address 0x0000", testString1);
    ...
    if (eepromPoll() == I2C_CODE_OK);//    printf("\n        - program cycle completed");
    else {
        printf("\nTest #1 - write string '%s' to address 0x0000", testString1);
...
//  printf("\nTest #3 - read string from address 0x0000");
    ...
    if (errorCode == I2C_CODE_OK) {
        if (strlen(testBuf) == sizeof(testString1) - 1);//      printf("\n        - string is '%s'", testBuf);
        else {
            printf("\nTest #3 - read string from address 0x0000");
            printf("\n        - wrong length (read string is %d characters long)! %d", strlen(testBuf),
                   sizeof(testString1) - 1);

            printf("\n%d,%d,%d,%d,%d %d,%d,%d,%d,%d %d",
                   testBuf[0], testBuf[1], testBuf[2], testBuf[3], testBuf[4], testBuf[5], testBuf[6], testBuf[7],
                   testBuf[8], testBuf[9], testBuf[10]);
...
//  printf("\nTest #4 - read string from address 0x0040");
    ...
    if (errorCode == I2C_CODE_OK) {
        if (strlen(testBuf) == sizeof(testString2) - 1);//      printf("\n        - string is '%s'", testBuf);
        else {
            printf("\nTest #4 - read string from address 0x0040");
...
//  printf("\nTest #6 - read string from address 0x0000");
    errorCode = eepromPageRead(0x0000, testBuf, MAX_LENGTH);
    if (errorCode == I2C_CODE_OK) {
        if (strlen(testBuf) == sizeof(testString2) - 1 + 4);//      printf("\n        - string is '%s'", testBuf);
        else {
            printf("\nTest #6 - read string from address 0x0000");
            printf("\n        - wrong length (read string is %d characters long)!", strlen(testBuf));
            eepromTestResultOK = FALSE;
